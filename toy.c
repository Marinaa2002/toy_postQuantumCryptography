...
// Function to fill an array with small random values
static void toy_fill_small(short *buf, int n) {
    for (int k = 0; k < n; ++k) {
        // Generate a small random value using Hamming weight
        short val = rand() & 0xF;
        val = (val >> 1 & 1) - (val & 1);
        if (val < 0)
            val += TK_Q;
...
// Function to perform polynomial multiplication naively
...
// Function to perform matrix transpose times vector multiplication
...
// Function to add two vectors with optional negation of the second vector
...
// Function to generate random elements for Toy encryption
...
// Function to encrypt a plaintext using Toy encryption scheme
...
// Function to decrypt Toy ciphertext and check against secret key
